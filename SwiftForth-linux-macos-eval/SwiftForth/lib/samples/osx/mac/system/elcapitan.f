...
Probably only initialisers (and some settings?) should be done on the main
thread to avoid flickering while resizing a window. Experimenting will clarify.

So far the following has been observed for NSWindow instances:
Main thread prefered -> initialisation, running modal
...
sizing, queries (@frame), property (title, transparency, background) settings 
...
cr .( temporary El Capitan fixes loaded)
