...
calculates a ratio of 31-bit numbers that very closely approximates
the original ratio.  Such 31-bit ratios may then be used with */ for
...
used earlier. In all cases it will produce the same or better ratios
for the examples in Starting Forth.  Method derives from Euclid.

RATIO requires that both of its arguments be 63-bit unsigned numbers.
It returns a pair of 31-bit unsigned numbers in the same order that
are a darned good approximation to the first pair.  Data management
will some day be cleaned up if we ever get a  D/  that's fast enough
to make this whole procedure attractive for application rather than
design time use.
--------------------------------------------------------------------- }
...
18.84955592 6.00000000 RATIO CR . .    \ Pi, gives 235619449/75000000

19.02797280 7.00000000 RATIO CR . .    \  e, gives 11892483/4375000
