...
TRUE VALUE ?TEST-CODE           \ obsolete, for backward compatibility
...
MAKE-FLOOR   \ Set SwiftForth floating point option to truncate on integer conversion
...
: FTO ( -- )   ( R: float -- )   \ Usage: ( f) FTO <name>
   '
   STATE @ IF
      POSTPONE LITERAL POSTPONE >BODY POSTPONE DF!
   ELSE
      >BODY DF!
   THEN ; IMMEDIATE
...
: DSMATRIX   ( cell_size -- )
  CREATE  0 , , TYPE-ID ,
  DOES>  DUP [ 2 CELLS ] LITERAL + @ SWAP @ [ 2 CELLS ] LITERAL +
;
