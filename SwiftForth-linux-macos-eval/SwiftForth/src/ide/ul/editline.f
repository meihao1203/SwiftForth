...
This file implements enhanced console version of ACCEPT for command-
line input, editing, and history.
==================================================================== }
...
: /LBUFFER ( -- )    |LBUF| 256 + ALLOCATE THROW TO LBUF
   LBUF |LBUF| 256 + ERASE ;
...
: SAVE-LBUF ( -- )
   #LBUF IF  S" HOME" FIND-ENV IF
      R-BUF  2DUP R@ PLACE  S" /.SwiftForth_history" R@ APPEND
      R> COUNT R/W CREATE-FILE 0= IF
         LBUF #LBUF 0 DO  COUNT 2DUP 4 PICK WRITE-LINE DROP +  LOOP DROP
   DUP CLOSE-FILE DROP  THEN DROP  THEN 2DROP  THEN ;

: RESTORE-LBUF ( -- )
   S" HOME" FIND-ENV IF  R-BUF  2DUP R@ PLACE  S" /.SwiftForth_history" R@ APPEND
      R> COUNT R/O OPEN-FILE 0= IF  LBUF  BEGIN
         DUP 1+ 256 3 PICK READ-LINE DROP WHILE
      OVER C!  COUNT +  REPEAT DROP  0 SWAP C!  DUP CLOSE-FILE DROP
...
: /COMPLETE ( a n # -- a n # )
   THIRD PATTERN COUNT TUCK COMPARE(CS)
...
CREATE INSERTING  -1 ,     \ 0=overstrike, nonzero=insert
...
: A-TYPE< ( a n -- )
  ?DUP IF
     GET-XY  2SWAP TYPE  0 OVER AT-XY  AT-XY  EXIT
   THEN DROP ;
...
: A-MATCH-LINE?  ( a n # -- a n # flag )  \ True = Match
   THELINE @ NTH-LINE   PATTERN COUNT  ROT MIN TUCK COMPARE(CS) 0= ( Match? ) ;
...
: A-SHOW-HIST  ( a n # -- a n # )  A-CANCEL  10 .HISTORY ;
...
: (STUFF) ( a n # a n -- a n # )
   2DUP TYPE  >R >R  2DROP  R> OVER R@ CMOVE  R> -1 ;

: "BUILD" ( -- ) ( a n # -- a n # )
   OVER IF  A-CANCEL  THEN  S" INCLUDE build" (STUFF) ;

: "DEBUG" ( -- ) ( a n # -- a n # )
   OVER IF  A-CANCEL  THEN  S" INCLUDE debug" (STUFF) ;
...
: A-QUIT ( a n # -- a n # )   DUP IF  7 EMIT EXIT  THEN  BYE ;

: BLANK? ( a n -- flag )      BL SKIP NIP 0= ;

: A-EOF ( a n # -- a n # )
  >R  2DUP BLANK?  R> SWAP IF A-QUIT ELSE A-DELETE THEN ;

: A-DEL-LEFTWORD ( a n # -- a n # )
  DUP >R  A-LEFTWORD  R> OVER - 0 ?DO A-DELETE LOOP ;

: E-STROKE ( a n # echar -- a n # )
   CASE
      VERASE OF A-BACKSPACE ENDOF
      [CTRL] H OF A-BACKSPACE ENDOF
      VKILL OF A-CANCEL ENDOF
      $1B OF A-CANCEL ENDOF             \ esc
      K-HOME OF A-HOME ENDOF
      [CTRL] A OF A-HOME ENDOF
      K-END OF A-END-REPAT ENDOF
      [CTRL] E OF A-END-REPAT ENDOF
      [CTRL] I OF A-COMPLETE ENDOF      \ tab
      [CTRL] M OF A-CR ENDOF            \ cr
      K-DELETE OF A-DELETE ENDOF
      K-LEFT OF A-LEFT-REPAT ENDOF
      K-RIGHT OF A-RIGHT-REPAT ENDOF
      K-UP OF A-UP-ARROW ENDOF
      K-DOWN OF A-DN-ARROW ENDOF
      K-CTRL_LEFT OF A-LEFTWORD ENDOF
      K-CTRL_RIGHT OF A-RIGHTWORD ENDOF
      K-INSERT OF A-TOGGLE-INSERT ENDOF
      K-F3 OF A-APPEND-LAST ENDOF       \ F3 - Append Last to EOL
      VEOF OF A-EOF ENDOF
      VWERASE OF A-DEL-LEFTWORD ENDOF
      [CTRL] B OF A-LEFT ENDOF
      [CTRL] F OF A-RIGHT ENDOF
      [CTRL] K OF A-DEL-EOL ENDOF
      [CTRL] L OF A-CANCEL PAGE ENDOF
      K-F8 OF  "BUILD"  ENDOF
      K-F9 OF  "DEBUG"  ENDOF
      DUP 32 256 WITHIN IF  CHARACTER REPATTERN DUP  THEN
   ENDCASE ;
...
' E-ACCEPT 'ACCEPT !	\ Reassign the console terminal personality's ACCEPT vector
