...
: PATH-REQUIRES ( z-addr addr u -- z-addr false | true )
   R-BUF  +ROOT  R@ ZPLACE  PUSHPATH  R> SetCurrentDirectory IF
      DUP IS-FILE IF  ZCOUNT ['] INCLUDED CATCH
...
: (REQUIRES) ( z-addr -- )
   ...
   S" %SwiftForth\lib\options\win32" PATH-REQUIRES ?EXIT
   S" %SwiftForth\lib\samples" PATH-REQUIRES ?EXIT
   S" %SwiftForth\lib\samples\win32" PATH-REQUIRES ?EXIT
