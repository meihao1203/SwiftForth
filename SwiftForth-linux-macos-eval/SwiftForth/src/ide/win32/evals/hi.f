...
INCLUDE %SwiftForth\src\ide\win32\winmaker
...
INCLUDE %SwiftForth\src\ide\win32\startup

-? : HI ( -- )   ." Already loaded" ;

INCLUDE %SwiftForth\src\ide\win32\evals\splash.f        \ Eval splash screen
S" SwiftForth **EVALUATION**" TITLE ZPLACE              \ Eval window title
...
{ --------------------------------------------------------------------
Note the GILD above to protect the electives loaded so far.  Below, we
define a marker, allot some space for safety margin, load the minimal
turnkey utility, get the address of PROGRAM, execute the marker to
roll back the dictionary (hence the safety pad), then EXECUTE to save
the turnkey.  This is all done so the evaluation turnkey has the
important limitation of not being able to save a turnkey.

SPLASH is set to NOOP after the real version has been saved in the
turnkey.  This enables us to exit without the nag screen after we save
the turnkey.
-------------------------------------------------------------------- }

MARKER FIN  $1000 ALLOT
INCLUDE %SwiftForth\src\ide\win32\save.f
' PROGRAM ( *)  FIN
CD %SwiftForth\bin
( *) EXECUTE sf-eval.exe sf.ico
